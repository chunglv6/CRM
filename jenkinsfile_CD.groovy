    ...
    env.project_version = "${env.GIT_TAG_DEPLOY}_GitTag_${env.config_git_branch}"
        if(deployer.contains(deployInput.submitter)){
            ...
            stage('5. Pull images and push to habor 10.60.117.196'){
			   withCredentials([usernamePassword(credentialsId: 'duybvk-docker', usernameVariable: 'username',
                    ...
                    sh """
                        ...
                        docker tag 10.60.156.72/etc/crm-backend:$env.GIT_TAG_DEPLOY 10.60.117.196/etc/crm-backend:${env.project_version}
                    """
                }
                ...
                sh """
                    ...
                    docker --config ~/.docker/.truongdx8_habor_196 push 10.60.117.196/etc/crm-backend:${env.project_version}
                    docker rmi 10.60.117.196/etc/crm-backend:${env.project_version}
                """
                }
            }
            stage('6. Deploy to Productions'){
                ...
                jenkinsfile_CI.release2k8s("${env.project_version}","etc-idc","crm-backend","config_117_191")
            }
...
def deployToPreRelease() {
    ...
    stage('3. Pull images and push to habor 10.60.117.196'){
        withCredentials([usernamePassword(credentialsId: 'duybvk-docker', usernameVariable: 'username',
            ...
            sh """
                docker --config ~/.docker/.duybvk login -u ${username} -p '${password}' 10.60.156.72
                docker --config ~/.docker/.duybvk pull 10.60.156.72/etc/crm-backend:$versionRunningStaging
                docker tag 10.60.156.72/etc/crm-backend:$versionRunningStaging 10.60.117.196/etc/crm-backend:${env.project_version}
            """
        }
        withCredentials([usernamePassword(credentialsId: 'truongdx8-habor-196', usernameVariable: 'username',
            passwordVariable: 'password')]){
    ...
    stage('5. Deploy to Pre release'){
        ...
        release2k8s("${env.project_version}","etc-idc","crm-backend","config_117_191","etc-release")
    }
    stage('6. Run Automaions Test in IDC'){
        echo "Run automations Test"
    }
    currentBuild.result = "SUCCESS"
}
    ...
    def resultStaging = ''
        if(deployer.contains(deployInput.submitter)){
            ...
            stage('6. Deploy to Productions'){
                echo "Tag: ${env.GIT_TAG_DEPLOY}"
                echo "Version Deploy: ${env.project_version}"
                jenkinsfile_CI.release2k8s("${env.project_version}","etc-vtnet","crm-backend","config_247_11")
            }
            currentBuild.result = "SUCCESS"
        }else {
            stage("Cancel deploy process") {
