...
def packageServicesAndUploadToRepo(groupId, artifactId, moduleName){
    ...
    stage('Upload artifact to Nexus server'){
        ...
        def uploadSuccessComment = "<b>Build & package Artifact Results - " +
            ...
            nexusArtifactUploader artifacts: [[artifactId: "${artifactId}_${moduleName}", classifier: '', file: "target/serviceEtc-0.0.1-SNAPSHOT.jar", type: 'jar']], credentialsId: 'ee7052bb-acda-4b89-826d-55a496501a1c', groupId: "${groupId}", nexusUrl: '10.60.156.26:8081', nexusVersion: 'nexus3', protocol: 'http', repository: 'msbuild', version: "1.${BUILD_NUMBER}"
...
def pushImageToDockerRepo(service,version){
    ...
    stage ("Push image to Docker Repository"){
		sh """
			echo "Push ${service}"
            docker --config ~/.docker/.duybvk push 10.60.156.72/etc/${service}:${version}
...
def deploy_module_web(server,groupId,artifactId){
    ...
    sh """
        pwd
        ansible-playbook cicd/deploy/deploy_etc_crm-backend_${server}.yml -e groupId=${groupId} -e artifactId=${artifactId} -e BUILD_NUMBER=${BUILD_NUMBER}
    """
}
...
def release2k8s(version,enviroment,service,configFile){
    ...
    try {
        ...
        dir("${env.buildFolderDeployResult}/k8s/${enviroment}"){
            ...
            def checkProcessFrontendRunning = sh(returnStdout: true, script: "kubectl -n etc --kubeconfig='${configFile}' get pods --sort-by=.status.startTime | grep 'crm-frontend' | tail -n 1 | awk '{print \$3}'").trim()
            echo "checkProcessFrontendRunning: $checkProcessFrontendRunning crm-front-end"
            if(checkProcessFrontendRunning == "Running"){
                env.STAGING_PORT_FRONT_END = sh(returnStdout: true, script: "kubectl -n etc --kubeconfig='${configFile}' get svc | grep 'crm-frontend' | awk '{print \$5}' | grep -o '[[:digit:]]*' | tail -n 1").trim()
...
def deployToPreRelease(version){
    ...
    stage('Pull images and push to habor 10.60.117.196'){
        try{
            withCredentials([usernamePassword(credentialsId: 'duybvk-docker', usernameVariable: 'username',
                ...
                sh """
                    docker --config ~/.docker/.duybvk login -u ${username} -p '${password}' 10.60.156.72
                    docker --config ~/.docker/.duybvk pull 10.60.156.72/etc/crm-backend:$version
                    docker tag 10.60.156.72/etc/crm-backend:$version 10.60.117.196/etc/crm-backend:${version}_preRelease
                """
            }
            withCredentials([usernamePassword(credentialsId: 'truongdx8-habor-196', usernameVariable: 'username',
                ...
                sh """
                    ...
                    docker --config ~/.docker/.truongdx8_habor_196 push 10.60.117.196/etc/crm-backend:${version}_preRelease
                    docker rmi 10.60.117.196/etc/crm-backend:${version}_preRelease
                """
            }
    ...
    stage('4. Deploy to Pre release'){
        ...
        release2k8sPreRelease("${env.project_version}","etc-idc","crm-backend","config_117_191","etc-release")
    }
}
/*
    ...
    tasks['Package and Build Artifact'] = {
        node("$env.node_slave") {
            stage('Build Package & Image Docker'){
                buildService("PUSH","${version}","10.60.156.72","crm-backend")
            }
        }
    }
    parallel tasks
    ...
    if(env.gitlabBranch == env.STAGING_BRANCH){
        uploads['Push Image To Repo Docker'] = {
            node("$env.node_slave"){
                stage('Push Image To Repo Docker'){
                    pushImageToDockerRepo("crm-backend","${version}")
                }
            }
        }
        parallel uploads
        ...
        deploys['Deploy to K8s'] = {
            node("$env.node_slave"){
               stage('Deploy to K8s'){
                    echo 'deploy to K8s'
                    release2k8s("${version}","etc-staging","crm-backend","config_108_172")
                    sleep(30)
                }
            }
        }
        ...
        deploys['Execute update DB DEV'] = {
         ...
         deploys['Deploy to K8s'] = {
            node("$env.node_slave"){
               stage('Deploy to K8s'){
                    echo 'deploy to K8s'
                    release2k8s("${version}","etc","crm-backend","config_155_31")
                }
            }
        }
        parallel deploys
...
def buildMergeRequest() {
    ...
    tasks['Package and Build Artifact'] = {
        node("$env.node_slave") {
            stage('Build Package & Image Docker'){
                buildService("MERGE","${version}","10.60.156.72","crm-backend")
            }
        }
    }
    parallel tasks
